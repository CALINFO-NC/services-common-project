...
class KeycloakJWTProcessor implements JWTProcessor<SecurityContext> {
    ...
    public KeycloakJWTProcessor(JWTClaimsSetAwareJWSKeySelector<SecurityContext> keySelector){
        ConfigurableJWTProcessor<SecurityContext> defaultJwtProcessor = new DefaultJWTProcessor();
        defaultJwtProcessor.setJWTClaimsSetAwareJWSKeySelector(keySelector);
